...
#run("rsync -avz --include='alpha*.so' --include='huanli_alpha*.so' --exclude='*' ~/alpha/build/out/lib/*  huan@%s:~/notebooks" % hostalias)
...
run("ssh huan@%s 'mkdir -p ~/alpha/home/share/store_tools'" % hostalias)
run("rsync -avz ~/alpha/public/home/share/store_tools/* huan@%s:~/alpha/home/share/store_tools" % hostalias)
run("rsync -avz ~/alpha/scripts/* huan@%s:~/alpha/scripts" % hostalias)
